...
from core.repair_agent import AgentLoopError, AgentTurn, RepairAgent, ToolCall
...
class Tools:
    ...
    def inspect(self, path):
        return {"path": path, "content": (self.root / path).read_text(encoding="utf-8")}
...
class Planner:
    ...
    def next_turn(self, *, failure, observations):
        ...
        if self.i == 1:
            return AgentTurn((
                ToolCall("inspect", {"path": "module.py"}),
                ToolCall("search", {"query": "VALUE mismatch"}),
            ))
        if self.i == 2:
...
def make_agent(tmp_path: Path, planner):
    ...
    return RepairAgent(
        planner=planner, tools=Tools(tmp_path), worker=worker, base_sha=base,
