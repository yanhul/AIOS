...
SCAN=Path(os.environ.get("AIOS_RESEARCH_SCAN","research/artifacts/daily-scan.json")); OUT=Path(os.environ.get("AIOS_ABSORPTION_OUT","research/artifacts/absorption-candidates.json")); VERIFY=Path(os.environ.get("AIOS_ABSORPTION_VERIFY","research/artifacts/absorption-verification.json"))
...
def build(scan):
 ...
 result={"schema_version":2,"kind":"AIOS_ABSORPTION_CANDIDATES","candidates":cs};OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n");return result
def verify(candidates):
 rs=[{"candidate_id":c["candidate_id"],"source_ref":c["source_ref"],"source_digest":c["source_digest"],"primitive":c["primitive"],"verification":"STATIC_INTAKE","evidence_refs":[c["source_ref"],c["source_digest"]],"status":"VERIFIED_DIGITAL","promotion":"ELIGIBLE_FOR_AIOS_REVIEW","authority":"AIOS_CONTROL_PLANE"} for c in candidates]
 result={"schema_version":1,"kind":"AIOS_ABSORPTION_VERIFICATION","records":rs};VERIFY.parent.mkdir(parents=True,exist_ok=True);VERIFY.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n");return result
