...
DEFAULT_SCAN=Path("research/artifacts/daily-scan.json")
DEFAULT_OUT=Path("research/artifacts/absorption-candidates.json")
DEFAULT_VERIFY=Path("research/artifacts/absorption-verification.json")
...
def build(scan):
 ...
 out=_path("AIOS_ABSORPTION_OUT",DEFAULT_OUT)
...
def verify(candidates, *, run_id=None):
 ...
 out=_path("AIOS_ABSORPTION_VERIFY",DEFAULT_VERIFY)
 out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n"); return result
if __name__=="__main__":
 scan=_path("AIOS_RESEARCH_SCAN",DEFAULT_SCAN)
 verify(build(json.loads(scan.read_text(encoding="utf-8")))["candidates"])
