"""Materialize verified AIOS-native absorption records without mutating source/policy.

ABSORBED means the independently verified primitive/adaptation has been admitted
to the durable AIOS absorption registry. It does not vendor source code or edit
AIOS implementation, policy, or promotion criteria automatically.
"""
...
def absorb(executor, verification, promotion, *, run_id):
    ...
    for d in decisions:
        ...
        if not e or not v:
            blocked.append({"candidate_id":cid,"verdict":"HOLD","reason":"missing executor or independent-verification lineage"})
            continue
        records.append({"candidate_id":cid,"run_id":run_id,"source_ref":e["source_ref"],
            ...
            "test_digest":v["test_digest"],"claim":v["claim"],
            "primitive":e["adaptation"]["primitive"],"signals":e["adaptation"]["signals"],
            "proposal":e["adaptation"]["proposal"],"target_surface":e["adaptation"]["target_surface"],
            "absorption_kind":"AIOS_NATIVE_PRIMITIVE_REGISTRY","status":"ABSORBED",
            "external_code_copy":False,"external_code_execution":False,"source_mutation":False,
            "authority":"AIOS_CONTROL_PLANE","verification":"INDEPENDENT_STRUCTURAL_CONFORMANCE"})
    expected=len(promotion.get("decisions",[]))+len(promotion.get("blocked",[]))
    result={"schema_version":1,"kind":"AIOS_ABSORPTION","run_id":run_id,
        "candidate_count":expected,"absorbed_count":len(records),"deferred_count":expected-len(records),
        "records":records,"blocked":blocked+[{"candidate_id":x.get("candidate_id"),"verdict":"DEFER",
...
if __name__=="__main__":
    ...
    r=absorb(e,v,p,run_id=os.environ.get("GITHUB_RUN_ID",p.get("run_id","local")))
    raise SystemExit(0 if r["overall"] in {"PASS","PASS_WITH_HOLDS"} else 1)