...
def _provider_authorized(contract, provider_name):
    return any(isinstance(ref, str) and ref.split("@", 1)[0] == provider_name for ref in contract.get("capabilities", []))

def _submit_runtime(runtime, method, effect, attempt_id, provider_name, **kwargs):
    if runtime is None: return None
    if not hasattr(runtime, method): raise ValueError(f"durable runtime must implement {method}")
...
def validate_receipt(receipt, effect, attempt_id, provider_name):
    if not isinstance(receipt, ProviderReceipt): raise ValueError("provider must return ProviderReceipt")
    for name, value in (("receipt.provider", receipt.provider), ("receipt.effect_id", receipt.effect_id), ("receipt.attempt_id", receipt.attempt_id), ("receipt.provider_operation_id", receipt.provider_operation_id)): _text(value, name)
    if receipt.effect_id != effect["effect_id"]: raise ValueError("receipt effect binding mismatch")
    if receipt.attempt_id != attempt_id: raise ValueError("receipt attempt binding mismatch")
    if receipt.provider != provider_name: raise ValueError("receipt provider binding mismatch")
...
def execute_attempt(aios_dir, contract, effect, actor, adapter, attempt_id):
    ...
    if not isinstance(effect, dict) or not effect: raise ValueError("effect must be a non-empty dict")
    if effect.get("state") != "DISPATCHED": raise RuntimeError("effect must be DISPATCHED before execute_attempt")
    if effect.get("actor") != actor: raise PermissionError("effect actor does not match execution actor")
    if effect.get("attempt_id") != attempt_id: raise RuntimeError("attempt_id does not match dispatched effect")
    ...
    if "external_effect" not in contract.get("allowed_effects", []): raise PermissionError("external effect is not authorized by contract")
    try:
...
def execute_retry_attempt(aios_dir, contract_id, permit_id, effect, actor, adapter, attempt_id, attempt, durable_runtime: DurableRuntime | None = None):
    _text(actor, "actor"); _text(attempt_id, "attempt_id")
    if not isinstance(effect, dict) or not effect: raise ValueError("effect must be a non-empty dict")
    if not isinstance(attempt, int) or isinstance(attempt, bool) or attempt < 2: raise ValueError("retry attempt must be an integer >= 2")
    ...
    if attempt > max_attempts: raise PermissionError("retry exceeds contract max_attempts")
    provider_name = _text(getattr(adapter, "name", None), "adapter.name")
    if not _provider_authorized(contract, provider_name): raise PermissionError("provider capability is not authorized by contract")
