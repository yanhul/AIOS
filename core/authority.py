...
def _resolve_policy(aios_dir,contract):
 try:
  resolve_policy(aios_dir,contract["policy_digest"])
 except ValueError as exc:
  raise TransitionError(f"policy authority rejected contract: {exc}") from exc
def persist_contract(aios_dir,contract):
 validate_contract(contract); _resolve_capabilities(aios_dir,contract); _resolve_policy(aios_dir,contract); cid=contract_identity(contract); record=dict(contract); record["record_type"]="EXECUTION_CONTRACT"; record["contract_id"]=cid; recover_pending(aios_dir); path=_path(aios_dir,CONTRACTS_DIR,cid)
...
def persist_attestation(aios_dir,contract,permit,secret):
 ...
 validate_contract(contract); verify_permit(contract,permit); _resolve_capabilities(aios_dir,contract); _resolve_policy(aios_dir,contract); attestation=issue_attestation(contract,permit,secret); recover_pending(aios_dir); path=_path(aios_dir,ATTESTATIONS_DIR,permit["permit_id"])
 if os.path.exists(path):
  existing=_load(path)
