...
_GATEWAY_FIELDS = ("target_sha", "evidence_ref", "lineage_ref", "idempotency_key", "attempt_fence")
...
class GovernedRuntimeExecutor(Executor):
    ...
    def act(self, decision: Any, state: Mapping[str, Any]) -> Any:
        ...
        binding = dict(self.gateway_binding)
        missing = [field for field in _GATEWAY_FIELDS if field not in binding]
        if missing: raise PermissionError(f"Gateway binding missing required fields: {', '.join(missing)}")
        result = execute(self.aios_dir, self.contract_id, self.permit_id, operation_id, self.actor, self.adapter,
                         binding["target_sha"], binding["evidence_ref"], binding["lineage_ref"],
                         binding["idempotency_key"], binding["attempt_fence"])
...
def run_governed_execution(*, executor: GovernedRuntimeExecutor, store: StateStore, policy: LoopPolicy) -> Mapping[str, Any]:
    ...
    if policy.policy_digest != contract["policy_digest"]: raise PermissionError("execution policy digest does not match governing contract")
    if policy.max_steps > contract["max_attempts"]: raise PermissionError("execution budget exceeds governing contract max_attempts")
    governing_terminal_states = frozenset(contract["terminal_states"])
    if not governing_terminal_states: raise PermissionError("governing execution must define terminal states")
    if policy.terminal_states != governing_terminal_states: raise PermissionError("execution terminal states do not match governing contract")
    if policy.failure_state not in governing_terminal_states: raise PermissionError("execution failure state is not authorized by governing contract")
    if policy.budget_exhaustion_state not in governing_terminal_states: raise PermissionError("budget exhaustion state is not authorized by governing contract")
    ...
    bound_policy = LoopPolicy(max_steps=policy.max_steps, terminal_evaluator=policy.terminal_evaluator,
                              action_authorizer=policy.action_authorizer, resume_validator=validate_resume,
                              policy_digest=contract["policy_digest"], terminal_states=governing_terminal_states,
                              budget_exhaustion_state=policy.budget_exhaustion_state, failure_state=policy.failure_state)
    loaded = store.load()
    if loaded is None:
